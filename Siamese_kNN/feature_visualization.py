...
"""
This script is used to visualize the embedding of the test set via TSNE.
"""
# %% load data
...
support_label = np.arange(120) // 20
...
for label in range(6):
    ...
    label_collect[int(label)] = pred_label[ele_index]
    plt.plot(data_dic[int(label)][:, 0], data_dic[int(label)][:, 1], '^', label='Class ' + str(label + 1), markersize=3)
plt.plot(support_norm[:, 0], support_norm[:, 1], 'oy', label='Training set', markersize=3., markeredgecolor='b')
...
plt.legend(prop={'family': 'Times New Roman', 'size': 10}, loc='upper right', ncol=2, markerscale=1.3, fancybox=False)
plt.yticks([])
plt.xticks([])
plt.tight_layout()
plt.show()
